...
function useFetchMovieTorrents({ imdbId, title }) {
  ...
  useEffect(() => {
    ...
    async function fetchAll(retries = 3) {
      ...
      try {
        ...
        mergedData = mergedData.filter((value) => {
          return value.imdbId === imdbId;
        });
...
export default useFetchMovieTorrents;
